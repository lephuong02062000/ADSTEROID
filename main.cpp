...
const int METEOR_FREQ = 20; // cái này chỉnh tần xuất nó ra thêm đá
...
bool Init()
{
	...
	gWindow = SDL_CreateWindow("SPACECRAFT AND METEOR", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
	if (gWindow == NULL)
	{
...
void NewGame() {
	...
	std::vector<Bullet*>* Ammo = new std::vector<Bullet*>;
	std::vector<Meteor*> RocksRain;
	Object Explosion(gRenderer, "explosion.png");
	Spacecraft* spacecraft = new Spacecraft(gRenderer, "spacecraft.png", Ammo);
	...
	while (!isQuit)
	{
		...
		if (delay > 0) delay--;
			...
			int n = rand() % 5 + 1; // tao ngau nhien 1 -> 5 thien thach // cái này chỉnh mỗi lần tạo nó ra mấy viên
			while (n--)
			{
				delay = METEOR_FREQ;
				Meteor* newMeteor = new Meteor(gRenderer, "meteor.png", rand() % 10 + 1); // cái cuối chỉnh tốc dộ của đá bay
