...
void Meteor::Update(std::vector<Bullet*>* Ammo, Score& score)
{
	for (int i = 0; i < Ammo->size(); i++)
	{
		...
		if (inside(Rect, pBullet->getRect())) {
			...
			SDL_Delay(50);
			Rect.y = -100; // lam vien da bay ra ngoai man hinh, sau do tiep tuc roi lai cho cu // tang do kho cho game
			//Rect.y = 800; // lam vien da bay ra ngoai man hinh, sau do bi xoa bo
		}
	}
